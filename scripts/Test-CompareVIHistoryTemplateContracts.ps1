...
Assert-Match -Content $manualExplorationTemplate -Pattern '(?m)^\s*required:\s+true\s*$' -Message 'Manual exploration template must require explicit characterization modes.'
Assert-NotMatch -Content $manualExplorationTemplate -Pattern '(?m)^\s*default:\s+attributes,front-panel,block-diagram\s*$' -Message 'Manual exploration template must not ship a bundled compare-mode default.'
Assert-Match -Content $manualExplorationTemplate -Pattern '(?m)^\s*default:\s+collapse\s*$' -Message 'Manual exploration template must default to collapsed noise handling.'
...
Assert-Match -Content $manualExplorationWorkflow -Pattern 'Explicit characterization compare mode list for manual exploration' -Message 'Manual exploration workflow must describe characterization mode selection explicitly.'
Assert-Match -Content $manualExplorationWorkflow -Pattern '(?m)^\s*required:\s+true\s*$' -Message 'Manual exploration workflow must require explicit characterization modes.'
Assert-NotMatch -Content $manualExplorationWorkflow -Pattern '(?m)^\s*default:\s+attributes,front-panel,block-diagram\s*$' -Message 'Manual exploration workflow must not ship a bundled compare-mode default.'
Assert-Match -Content $manualExplorationWorkflow -Pattern '(?m)^\s*default:\s+collapse\s*$' -Message 'Manual exploration workflow must default to collapsed noise handling.'
...
Assert-Match -Content $releaseWorkflow -Pattern 'scripts/Invoke-CompareVIHistoryReleaseProof\.ps1' -Message 'Release workflow must run the staged release-proof gate.'
Assert-Match -Content $releaseWorkflow -Pattern 'name:\s+release-proof' -Message 'Release workflow must define a release-proof job.'
Assert-Match -Content $releaseWorkflow -Pattern 'COMPAREVI_NI_LINUX_IMAGE:\s+nationalinstruments/labview:2026q1-linux' -Message 'Release-proof must pin the NI Linux runtime image.'
Assert-Match -Content $releaseWorkflow -Pattern 'comparevi-history-release-proof-\$\{\{ github\.run_id \}\}' -Message 'Release workflow must upload and reuse the staged release-proof artifact.'
Assert-Match -Content $releaseWorkflow -Pattern 'staged-release-proof/release-assets/\*\.zip' -Message 'Publish job must publish the staged release-proof assets, not freshly repackaged assets.'
...
Assert-Match -Content $readme -Pattern 'operator must choose explicit characterization modes' -Message 'README must require explicit characterization mode selection for manual exploration.'
Assert-Match -Content $readme -Pattern 'noise_policy=collapse' -Message 'README must document the collapsed manual exploration noise-policy default.'
Assert-Match -Content $readme -Pattern 'characterization-only, not a certified developer decision surface' -Message 'README must state that manual exploration is still characterization-only.'
...
Assert-Match -Content $readme -Pattern 'dev-fast' -Message 'README must document the accelerated dev-fast local-review profile.'
Assert-Match -Content $readme -Pattern 'warm-dev' -Message 'README must document the warm-runtime local-review profile.'
Assert-Match -Content $readme -Pattern 'Profile proof' -Message 'README must document the explicit proof local-review profile.'
Assert-Match -Content $readme -Pattern 'elapsed timings' -Message 'README must document timing metadata for local-review.'
...
Assert-Match -Content $readme -Pattern 'stricter proof profile' -Message 'README must explain that local-proof stays on the proof profile.'
Assert-Match -Content $readme -Pattern 'Invoke-CompareVIHistoryReleaseProof\.ps1' -Message 'README must document the release-proof entrypoint.'
Assert-Match -Content $readme -Pattern 'comparevi-history/release-proof@v1' -Message 'README must document the release-proof receipt contract.'
Assert-Match -Content $readme -Pattern 'tests/results/release-proof' -Message 'README must document the default release-proof results root.'
Assert-Match -Content $readme -Pattern 'release-proof-summary\.md' -Message 'README must document the release-proof summary output.'
Assert-Match -Content $readme -Pattern 'nationalinstruments/labview:2026q1-linux' -Message 'README must document the pinned NI runtime in release-proof.'
